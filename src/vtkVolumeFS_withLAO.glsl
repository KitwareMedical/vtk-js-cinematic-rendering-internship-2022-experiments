...
{ 
  ...
  float jitter=texture2D(jtexture,gl_FragCoord.xy/128.).r;
...
vec3 sample_direction_uniform()
{
  ...
  float phi = acos(2.0 * random()-1.0) / 2.5;
...
float computeLAO(vec3 posIS, float opacity, vec3 lightDir, vec4 normal){
  ...
  if (normal.w > 0.0 && opacity > 0.05){
    ...
    {
      vec3 randomDir = inverseRotateBasis * sample_direction_uniform();
      float weight = 1.0 - dot(normalize(lightDir), normalize(randomDir));
      vec3 currPos = posIS;
      float transmittance = 1.0;
      for (int j = 0; j < kernelRadius ; j++){
        currPos += sampleDistanceIS * randomDir;
        // check if it's at clipping plane, if so return full brightness
        if (all(greaterThan(currPos, vec3(EPSILON))) && all(lessThan(currPos,vec3(1.0-EPSILON)))){
          float sampled_transmittance = 1.0 - texture2D(otexture, vec2(getTextureValue(currPos).r * oscale0 + oshift0, 0.5)).r;
    ...
    return clamp(total_transmittance / float(kernelSize), 0.4, 1.0); 
  } else {
    return 1.0;
  }
}
